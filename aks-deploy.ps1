...
$RESOURCEGROUP = "eburgan-dapr"
$LOCATION = "westeurope"
$AKS_NAME = "eburgandapr"
...
###  2 - set up blob storage for state
...
###  3 - set up Azure service bus for pub sub
$SERVICE_BUS = "eburganpubsub"
...
###  4 - install Dapr onto the cluster
...
###  5 - get containers pushed to docker

# ensure we've built all our containers
docker build -f .\frontend\Dockerfile -t omurucum/eburgan-dapr-frontend .
docker build -f .\catalog\Dockerfile -t omurucum/eburgan-dapr-catalog .
docker build -f .\ordering\Dockerfile -t omurucum/eburgan-dapr-ordering .
...
docker push omurucum/eburgan-dapr-frontend
docker push omurucum/eburgan-dapr-catalog
docker push omurucum/eburgan-dapr-ordering

# 6 - put connection strings into Kubernetes secrets
